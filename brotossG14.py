...
class Brotoss(BotAI):
    ...
    def __init__(self):
        ...
        self.visited_expansions = []
        self.scout_reached_enemy_base = False
    ...
    async def on_step(self, iteration):
        ...
        if iteration == 0:
            await self.chat_send("BrotossBot goes brrrrrrrrrrr")
        ...
        if(self.strategy == "StalkerPush"):
            ...
            if(self.defensePhase and self.units(UnitTypeId.ZEALOT).amount >= 2 and self.units(UnitTypeId.STALKER).amount >= 1):
                ...
                army = self.units.filter(lambda unit: unit.type_id not in {UnitTypeId.PROBE})
                for unit in army:
                    if(unit.distance_to(self.scout_location)>5 and unit.tag in self.scoutArmy):
...
# ================= BehaviourTree for the scoutArmy ======================================================
        if len(self.scoutArmy) != 0:
            print("Iteration: ", iteration)
            our_army_units = self.units.filter(lambda unit: unit.type_id not in {UnitTypeId.PROBE})
            #print("our_army_units", our_army_units)
            scoutArmy = self.scoutArmy
            #print("scoutArmy", scoutArmy)
            enemy_list = (self.enemy_units | self.enemy_structures).filter(lambda unit: unit.can_be_attacked)
            #print("enemy_list", enemy_list)
            our_pylons = self.structures(UnitTypeId.PYLON)
            #print("our_pylons", our_pylons)
            our_nexuses = self.structures(UnitTypeId.NEXUS)
            #print("our_nexuses", our_nexuses)
            scout_location = self.scout_location
            #print("scout_location", scout_location)
            expansion_sites = self._expansion_positions_list
            #print("expansion_sites", expansion_sites)
            visited_sites = self.visited_expansions
            #print("visited_sites", visited_sites)
            full_send_bool = self.scout_reached_enemy_base

            not_visited = list(set(expansion_sites) - set(visited_sites) - set(self.start_location))
            scout_location = not_visited[0]
            scout_near_us_first = 3

            # Start by scouting the closest expansion spot
            for el in not_visited:
                if (el.distance_to(self.start_location) < scout_location.distance_to(self.start_location)):
                    scout_location = el
                if len(self.visited_expansions) > scout_near_us_first:
                    if (el.distance_to(self.enemy_start_locations[0]) < scout_location.distance_to(self.enemy_start_locations[0])):
                        scout_location = el

            if len(enemy_list) == 0:
                for unit in our_army_units:
                    if(unit.tag in scoutArmy):
                        if (unit.position.distance_to(scout_location) < 7):
                            self.visited_expansions.append(scout_location)

            #scout_location = self.scout_location

            scout = scoutMap(self.game_info, our_army_units, scoutArmy, enemy_list, our_pylons, our_nexuses, scout_location, expansion_sites, visited_sites, self.enemy_structures, full_send_bool)
            getIntoBase = fullSend(self.game_info, our_army_units, scoutArmy, enemy_list, our_pylons, our_nexuses, scout_location, expansion_sites, visited_sites, self.enemy_structures, full_send_bool)
            attack = attackEnemy(self.game_info, our_army_units, scoutArmy, enemy_list, our_pylons, our_nexuses, scout_location, expansion_sites, visited_sites, self.enemy_structures, full_send_bool)
            retreatFromEnemy = retreat(self.game_info, our_army_units, scoutArmy, enemy_list, our_pylons, our_nexuses, scout_location, expansion_sites, visited_sites, self.enemy_structures, full_send_bool)
            armySequence = Sequence( [getIntoBase, retreatFromEnemy, scout, attack ])#, kite])#] )
    ...
    def Evaluate(self):
        ...
        while i < len(self.node_list):
            ...
            # Continue evaluating this behaviour otherwise
                ...
                #node._nodeState = node.Evaluate()
                # First behaviour failed, break and try again next iteration
                break
...
class scoutMap(BTNode):
    def __init__(self, game_info, units, scoutArmy, enemy_list, our_pylons, our_nexuses, scout_location, expansion_sites, visited_sites, enemy_structures, full_send_bool):
        self.game_info = game_info
        self.our_units = units
        self.scouts = scoutArmy
        self.enemy_units = enemy_list
        self.pylons = our_pylons
        self.nexuses = our_nexuses
        self.scout_this_location = scout_location
        self.expansion_sites = expansion_sites
        self.visited_sites = visited_sites
        self.enemy_structures = enemy_structures
        self.in_enemy_base = full_send_bool

    def Evaluate(self):
        min_distance = 100
        for enemy in self.enemy_units:
            for unit in self.our_units:
                if (unit.tag in self.scouts):
                    distance_to_enemy = unit.distance_to(enemy)
                    if (distance_to_enemy < min_distance):
                        min_distance = distance_to_enemy

        # If we have an enemy close (which can not attack us), we move on to next behaviour
        if (min_distance > 15):
            print("scoutMap -> State.SUCCESS")
            return State.SUCCESS
        # Else we can continue to scout the map
        else:
            for unit in self.our_units:
                if (unit.tag in self.scouts):
                    print("self.scout_this_location) = ", self.scout_this_location)
                    unit.attack(self.scout_this_location)
                    print("scoutMap -> State.FAILURE #2")
                    return State.FAILURE




class fullSend(BTNode):
    # We want to continue to KITE when possible, we should therefore return FAILURE if we CAN kite
    # return SUCCESS means to move on to next behaviour, we do this when the enemy is non-kiteable
    def __init__(self, game_info, units, scoutArmy, enemy_list, our_pylons, our_nexuses, scout_location, expansion_sites, visited_sites, enemy_structures, full_send_bool):
        self.game_info = game_info
        self.our_units = units
        self.scouts = scoutArmy
    ...
    def Evaluate(self):
        for unit in self.our_units:
            if (unit.tag in self.scouts):
                ...
                if (unit.name == "ZEALOT"):
                    if (5 < zealot.distance_to(self.enemy_start_locations[0]) < 27):
                        behind_enemy_base = self.enemy_start_locations[0]#.offset(self.enemy_start_locations[0] - self.start_location)
                        unit.move(behind_enemy_base)
                        print("fullSend ZEALOTS")
                """
                if (5 < unit.distance_to(self.enemy_start_locations[0]) < 20):

                    behind_enemy_base = self.enemy_start_locations[0]#.offset(self.enemy_start_locations[0] - self.start_location)
                    unit.move(behind_enemy_base)
                    print("fullSend -> State.FAILURE")
                    print("behind_enemy_base = ", behind_enemy_base)

                    return State.FAILURE

        print("fullSend -> State.SUCCESS")
        return State.SUCCESS
...
class attackEnemy(BTNode):
    ...
    def Evaluate(self):
        enemies = self.enemy_units.filter(lambda unit: unit.type_id not in {UnitTypeId.PROBE})
        enemy_fighters = enemies.filter(lambda u: u.can_attack) + self.enemy_structures({UnitTypeId.PHOTONCANNON})

        # If the enemy has fighters -> Move on to next behaviour (retreat)
        if enemy_fighters:
            print("attackEnemy1 -> State.SUCCESS")
            return State.SUCCESS
        else:
            print("attackEnemy1 -> State.RUNNING")
                        ...
                        if unit.ground_range > 1:
                            ...
                            if unit.weapon_cooldown == 0:
                                ...
                                if (
                                    ...
                                    # if more than 5 units friends are close, use distance one shorter than range
                                    # to let other friendly units get close enough as well and not block each other
                                    if len(self.units(UnitTypeId.STALKER).closer_than(7, unit.position)) >= 5:
                                        distance -= -1
            ...
            print("attackEnemy1 -> State.FAILURE")
            return State.FAILURE
...
class retreat(BTNode):
    ...
    def Evaluate(self):
        ...
        # If the enemy has fighters, we retreat
        if enemy_fighters:
            for unit in self.our_units:
                if (unit.tag in self.scouts):
                    enemy_pos = enemy_fighters.closest_to(unit).position
                    if (unit.distance_to(enemy_pos) < 15):
                        unit.move(unit.position.offset(unit.position - enemy_pos))

            print("retreat -> State.FAILURE")
            return State.FAILURE
        # If the enemy close to us is not an enemy -> move to next behaviour (attack)
        else:
            print("retreat -> State.SUCCESS")
            return State.SUCCESS
